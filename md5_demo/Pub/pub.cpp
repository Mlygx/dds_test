...
extern "C" int publisher_main(int domainId, int sample_count)
{
	...
	for (count = 0; (sample_count == 0) || (count < sample_count); ++count) {
		...
		{
			cout << "please standin message" << endl;
			cin >> instance->message;
			strcpy(instance->md5_message,MD5(instance->message).toStr().c_str());
			UserDataTypeTypeSupport::printMD5(instance->message);
