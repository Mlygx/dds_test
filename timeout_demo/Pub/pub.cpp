...
void UserDataTypeListener::on_data_available(DataReader* reader)
{
	...
	static UserDataTypeSeq data_seq;
...
extern "C" int publisher_main(int domainId, int sample_count)
{
	...
	if (instance->circulate_count == 0) {
		cout << "please stdin count and mes_size" << endl;
		cin >> count >> instance->message_size;
	}
		instance->message = new char[instance->message_size];
		auto s_now = std::chrono::system_clock::now();
		auto s_now_us = std::chrono::time_point_cast<std::chrono::microseconds>(s_now);
		
		auto value = s_now_us.time_since_epoch().count();
		std::time_t t = value / 1000000;
		std::size_t fractional_seconds = value % 1000000;
		char buffer[100];
		std::strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", std::localtime(&t));
		std::cout << buffer << "." << std::setfill('0') << std::setw(6) << fractional_seconds << std::endl;
		instance->start_time = buffer;
		/* 5. 创建一个监听器 */
...
/* 建议4：避免打算接收数据时创建datareader，接收数据后删除，该做法消耗资源，影响性能 */
	...
	/* 7. 主循环 ，发送数据 */
	for (instance->circulate_count = 0; instance->circulate_count < count; ++instance->circulate_count) {
		if (instance->circulate_count == 0) {
			//发送第一次数据
			...
			//判断是否已读
				...
				if (tmp != NULL) {
					...
					if (retcode != RETCODE_OK) {
						...
						fprintf(stderr, "%d : write  successfully . . \n", instance->circulate_count+1);
					}
					if ((instance->circulate_count + 1) == count) {
						auto now = std::chrono::system_clock::now();
						auto now_us = std::chrono::time_point_cast<std::chrono::microseconds>(now);
						auto value = now_us.time_since_epoch().count();
						static auto end_time = now_us;
						std::time_t t = value / 1000000;
						std::size_t fractional_seconds = value % 1000000;
						char buffer[100];
						std::strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", std::localtime(&t));
						std::cout << buffer << "." << std::setfill('0') << std::setw(6) << fractional_seconds << std::endl;
						instance->end_time = buffer;
						auto time_out = now_us-s_now_us;
						auto time_out_us = std::chrono::duration_cast<std::chrono::microseconds>(time_out);
						std::cout << "time_out = " << time_out_us.count() << " us" << std::endl;
						retcode = UserDataType_writer->write(*instance, instance_handle);
						if (retcode != RETCODE_OK) {
							fprintf(stderr, "write error %d\n", retcode);
						}
	...
	/* 9. 删除所有实例 */
	return subscriber_shutdown(participant) && publisher_shutdown(participant);
}
