...
extern "C" int publisher_main(int domainId, int sample_count)
{
	...
	std::cout << "please stdin mes_size and send_cout" << std::endl;
	std::cin >> instance->message_size >> count;
	instance->message = new char(instance->message[instance->message_size]);
	instance->pub_mes_size += sizeof(*instance);
	/* 7. 主循环 ，发送数据 */
	//创建新线程
	thread threadObj(UserDataTypeTypeSupport::thread_function, instance);
	for (instance->send_count = 0; instance->send_count<count ; ++instance->send_count) {
